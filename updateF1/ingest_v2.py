...
if __name__ == '__main__':
    ...
    if args.project_id and args.main_db:
        try:
            args.main_db.parent.mkdir(parents=True, exist_ok=True)
            _mcon = sqlite3.connect(str(args.main_db))
            cols = {r[1] for r in _mcon.execute("PRAGMA table_info(events)").fetchall()}
            if cols:
                # Existing main DB — migrate if needed and clear prior data for this project
                ...
                _mcon.commit()
            _mcon.close()
            # DBv2 creates events + media_registry tables if missing with the full schema
